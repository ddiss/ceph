...
function test_activate_journal_map_dmcrypt() {
    local to_prepare=$1
    local to_activate=$2
    local journal=$3
    local journal_p=$4
...
function activate_journal_dmcrypt_map_body() {
    local disk=$1
    local journal=$2
    local newdisk=$3
    local uuid=$($uuidgen)
    ...
    setup
    run_mon
    test_activate_journal_map_dmcrypt $disk ${disk}p1 $journal p1 $uuid $juuid not_plain || return 1
    kill_daemons
    umount /dev/mapper/$uuid || return 1
    teardown
}

function test_activate_journal_dmcrypt_map() {
    test_setup_dev_and_run activate_journal_dmcrypt_map_body
}
