...
TEST_F(LibRadosMiscPP, WriteSamePP) {
  ...
  ASSERT_EQ(0, ioctx.writesame("ws", bl, bl.length(), sizeof(full), 0));
  ...
  bl.clear();
  bl.append(buf, sizeof(buf));
  ASSERT_EQ(-EINVAL, ioctx.writesame("ws", bl, bl.length(),
				     (sizeof(buf) * 4) - 1, 0));
  ASSERT_EQ(-EINVAL,
	    ioctx.writesame("ws", bl, bl.length(), bl.length() / 2, 0));
  ASSERT_EQ(-EINVAL,
	    ioctx.writesame("ws", bl, 0, bl.length(), 0));
  /* write_len = data_len, i.e. same as write() */
  ASSERT_EQ(0, ioctx.writesame("ws", bl, sizeof(buf), sizeof(buf), 0));
}
