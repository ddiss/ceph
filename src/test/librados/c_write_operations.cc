...
TEST(LibRadosCWriteOps, CmpExt) {
  ...
  char mismatch_buf[sizeof(uint64_t) + sizeof(hi)];
  memset(mismatch_buf, 0, sizeof(mismatch_buf));
  op = rados_create_write_op();
  ASSERT_TRUE(op);
  rados_write_op_cmpext(op, "four", 4, 0, mismatch_buf, sizeof(mismatch_buf),
			&mismatch_len, &val);
  ...
  ASSERT_EQ(-EILSEQ, rados_write_op_operate(op, ioctx, "test", NULL, 0));

  ASSERT_EQ(mismatch_len, sizeof(uint64_t) + strlen("five"));
  uint64_t mismatch_off = le64_to_cpu(*(__u64 *)mismatch_buf);
  ASSERT_EQ(mismatch_off, 1);	// "four" and "five" match at offset 0
  ASSERT_EQ(0, memcmp(mismatch_buf + sizeof(uint64_t), "five",
		      mismatch_len - sizeof(uint64_t)));
  ASSERT_EQ(-EILSEQ, val);
